Now, populate the Right Panel (Content Panel) of the `/case-study-1` page with the following text. 

# Styling Rules:
*   Maintain the visual hierarchy (H1 for Title, H3 for section headers, bulleted lists).
*   Text wrapped in `**` should be styled as bold, and optionally given a subtle highlight color (like the neon green `#39FF14`) to draw the user's eye to key metrics and achievements.
*   Where it says `[EMBED FIGMA PROTOTYPE]`, create a full-width container (`w-full`, rounded corners) and embed the Figma iframe.
...
Now, populate the Right Panel (Content Panel) of the `/case-study-2` page with the following text. 
